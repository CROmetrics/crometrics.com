	...
	<div class="thumbnails <?php echo 'columns-' . $columns; ?>"><?php
		...
		foreach ( $attachment_ids as $attachment_id ) {
			...
			if ($adaptive_images === 'on' && $adaptive_images_async === 'on') {
				...
				$media_data = ' data-uniqueid="'.$attachment_id.'-'.big_rand().'" data-guid="'.$image_attributes->guid.'" data-path="'.$image_attributes->path.'" data-width="'.$image_metavalues['width'].'" data-height="'.$image_metavalues['height'].'" data-singlew="5" data-singleh="null" data-crop=""';
			}
